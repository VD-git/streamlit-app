...
class PokemonAgent:
    ...
    def stream_memory_responses(self, logger, user_input: str):
        """
        Streams the conversation with the Pokémon agent and retrieves the latest response generated by the model
        """
        messages = []
        for event in self.app.stream({"messages": [("user", user_input)]}, self.config, stream_mode = "updates"):
            for node_data in event.values():
                messages.extend([msg.content for msg in node_data.get("messages", [])])
                logger.info(f"\033[1m[Agent workflow streamed messages]:\033[0m {messages}")
        return messages[-1]
